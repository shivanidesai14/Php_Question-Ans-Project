...
<div >
  <h1>Your Questions</h1><hr style="  border-top: 5px solid black;">
</div>

  <?php
    
    $conn=new mysqli("localhost","root","","pro_question");
    $sql="select * from question_tbl where fk_email_id='". $_email ."'";
    $result=$conn->query($sql);
    while($row=$result->fetch_assoc())
    {
 echo '<div class="panel panel-default">';
echo   '<div class="panel-body">';
echo '<div class="panel panel-heading" style="background-color:rgb(215,233,213);font-size:25px;color:black; font-weight:bold;"><span class="glyphicon glyphicon-star"> ' .  $row["question_desc"] . '</div></span>';
   
    $tmp=$row["question_id"];
    $sql1="select a.*,u.* from answer_tbl a,user_tbl u where u.pk_email_id=a.fk_email_id and a.fk_question_id='". $tmp ."'";
    $res=$conn->query($sql1);
    while($ro=$res->fetch_assoc())
    {
     echo '<h4><img src="'. $ro["user_img"] .'" class="img-circle" width="70" height="70"></img>&nbsp&nbsp<b style="text-align:top;font-size:25px;">'. $ro["user_name"]  .'</b><p  style="float:right;font-size:25px;"><span class="glyphicon glyphicon-time  aria-hidden="true">'. $ro["ans_date"] .'</span></p></h4><br>';    
     echo '<div class="well well-default well-sm" style="color:black; font-size:22px;font-weight: bold;"><span class="glyphicon glyphicon-arrow-right"> ' .  $ro["ans_desc"] . '</span></a></div><hr style="border:1px dashed black">';
