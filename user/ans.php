...
<div class>
  ...
  <li role="presentation"  style="font-size:25px;"><a href="index.php">All Qusetions</a></li>
...
$_SESSION["id"]=$_id;
$obj=new database();
 $result=$obj->selectquestion($_id);
 $row=$result->fetch_assoc();
 $_name=$row["fk_email_id"];
 $obj=new database();
 $res=$obj->userget($_name);
...
//$sql="select a.*,u.* from answer_tbl a,user_tbl u where and a.fk_email_id=u.pk_email_id and a.fk_question_id='". $_id ."'";
...
echo '<div class="panel panel-heading" style="background-color:rgb(215,233,213);font-size:25px; font-weight:bold;"><span class="glyphicon glyphicon-star"> ' .  $row["question_desc"] . ' </span></div>';
$obj=new database();
$res1=$obj->ansget($_id);
echo '<center><a onclick="window.scrollTo(0,1000);" class="btn btn-lg btn-primary" style="color:white;">Give Answer</a></center>';
while($row2=$res1->fetch_assoc())
{
    $_mail=$row2["fk_email_id"];
  
    $obj=new database();
    $res5=$obj->userget($_mail);
    $row3=$res5->fetch_assoc();
    echo '<h4><img src="'. $row3["user_img"] .'" class="img-circle" width="70" height="70"></img>&nbsp&nbsp<b style="text-align:top;font-size:25px;">'. $row3["user_name"]  .'</b><p  style="float:right;font-size:25px;"><span class="glyphicon glyphicon-time  aria-hidden="true">'. $row2["ans_date"] .'</span></p></h4><br>';    
     echo '<div class="well well-default well-sm" style="color:black; font-size:22px;font-weight: bold;"><span class="glyphicon glyphicon-arrow-right"> ' .  $row2["ans_desc"] . '</span></a></div><hr style="border:1px dashed black">';
}
?>
</h4>
...
<form action="answer.php" method="post" enctype="multipart/form-Data">
  ...
  <textarea class="form-control" name="txtdesc" style="font-weight:bold;color:black;font-size:32px;" rows="5" placeholder="Your Answer" id="comment"></textarea>
</div>
 <script>
  ...
  <div class="form-group">
    <input type="file" style="font-weight:bold;color:black;" name="txtimg">
  </div>


<center><input type="submit" class="btn btn-primary btn-block" style="color:white;font-size:20px;" name="btnsubmit" value="Answer"></center>
</form>
</div>
</div>
</body>
